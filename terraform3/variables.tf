variable "name" {
  type = string
}

variable "GBL_CLASS_0" {
  type = string
}

variable "GBL_CLASS_1" {
  type = string
}

variable "SEC_ASSETS_PUBLIC" {
  type = string
}

variable "SEC_ASSETS_PII" {
  type = string
}

variable "personal_cidr_block" {
  type = string
}

variable "zscaler1_cidr_block" {
  type = string
}

variable "zscaler2_cidr_block" {
  type = string
}

variable "zscaler3_cidr_block" {
  type = string
}

variable "zscaler4_cidr_block" {
  type = string
}

variable "all_cidr_block" {
  type = string
}

variable "vpc_cidr_block" {
  type = string
}

variable "pubsub_a_cidr_block" {
  type = string
}

variable "pubsub_c_cidr_block" {
  type = string
}

variable "prisub_a_cidr_block" {
  type = string
}

variable "prisub_c_cidr_block" {
  type = string
}

variable "dbsub_a_cidr_block" {
  type = string
}

variable "dbsub_c_cidr_block" {
  type = string
}

variable "availability_zone1" {
  type = string
}

variable "availability_zone2" {
  type = string
}

variable "instance_type" {
  type = string
}

variable "ssh_port" {
  type = string
}

variable "ssh_description" {
  type = string
}

variable "http_port" {
  type = string
}

variable "http_description" {
  type = string
}

variable "protocol" {
  type = string
}
